...
namespace Nexus.Blood
{
    ...
    class BloodMetabolism 
    {
        ...
        public void BloodMetabolismCycle() //the main thirst method for using your blood as time goes on and giving you Bloodthirst
        {
            ...
            Helpers.ResetWater(ref Source.ParentObject.GetPart<Stomach>().Water);
        }
        ...
        void Overfed()
        {
            if (Source.Blood >= VITAE.BLOOD_PUKE && !Scan.SafeReturnProperty(Source.ParentObject, Flags.FRENZY))
            {
                Popup.Show("You overfed!");
                Helpers.Vomit(Source.ParentObject);
            }
        }
        ...
        void Bleeding()
        {
            ...
            {
                Source.Blood -= Scan.SafeReturnProperty(Source.ParentObject, Flags.FEED) ? VITAE.BLOOD_PER_BLOODLOSS_FEED : VITAE.BLOOD_PER_BLOODLOSS;
