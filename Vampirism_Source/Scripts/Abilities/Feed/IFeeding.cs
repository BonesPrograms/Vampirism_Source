...
namespace XRL.World.Effects
{
	...
	public abstract class IFeeding : Effect
	{
		...
		public IFeeding() => DisplayName = "feeding";
		...
		bool PlayerChoseNotToPuke(Vitae v) => v.Blood >= VITAE.FEED_PUKE_WARN && v.IDontWantToPuke(true);
		...
		public sealed override bool HandleEvent(AfterDieEvent E)
		{
			if (other.Object.IsPlayer())
			{
				...
				if (!Scan.SafeReturnProperty(other.Object, Flags.GO) && UI.Options.GetOptionBool(OPTIONS.HUMANITY) && (Scan.SafeReturnProperty(E?.Dying, Flags.INNOCENT) || friendly))
		...
		void MarkVictim()
		{
			...
			{                                       //but only feeding, anything else tracks back to the original player's humanity score
				if (Scan.SafeReturnProperty(other?.Object, Flags.INNOCENT)) //ALSO prevents a potential bug where you would lose humanity for killing someone
