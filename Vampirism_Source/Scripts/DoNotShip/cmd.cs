...
namespace XRL.World.Parts
{
    ...
    public class cmd : IPart
    {
        ...
        {
            Out().showbloodtype = Method(Out().showbloodtype, "showbloodtype");
        }
        ...
        {
            Out(out cmd cmd);
            cmd.showturns = Method(cmd.showturns, "showturns");
        }
        ...
        public void Properties(GameObject tgt, bool Stealthed, bool ActiveStealth)
        {
            string message = "";
            if (showStealthed)
                message += TextMaker(Stealthed, "Stealthed", 'B');
            if (ShowActiveStealthed)
                message += TextMaker(ActiveStealth, "ActiveStealth", 'b');
        ...
        {
            bool HumanityGameOver = Scan.SafeReturnProperty(tgt, Flags.GO);
            bool Feeding = Scan.SafeReturnProperty(tgt, Flags.FEED);
            bool Frenzying = Scan.SafeReturnProperty(tgt, Flags.FRENZY);
            bool Stealthy = Scan.SafeReturnProperty(tgt, Flags.STEALTH);
