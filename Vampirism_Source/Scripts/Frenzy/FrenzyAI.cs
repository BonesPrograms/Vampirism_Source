...
namespace XRL.World.Effects
{
    ...
    public class FrenzyAI : Effect
    {
        ...
        =>
            Object == base.Object
            && !InRange
            && !Scan.SafeReturnProperty(base.Object, Flags.FEED) //fun bug here. because frenzy never uses energy, if you are attacked by a group, you will stack feeding on all of them and become god. so we check for if FEED == false before swapping targets
            && Actor?.CurrentCell?.GetCombatTarget(base.Object) is not null
            && Scan.Applicable(Actor)
        ...
        void EndingTimer()
        {
            if (!gameover)
            {
                if (!activated && Scan.SafeReturnProperty(base.Object, Flags.FEED))
                    StartCountdown();
                if (activated)
                    Countdown();
            }
        ...
        {
            feedtime--;
            if (feedtime == 0 || !Scan.SafeReturnProperty(base.Object, Flags.FEED))
                Duration = 0;
        }
        ...
        {
            Vampirism part = base.Object.GetPart<Vampirism>();
            part.CooldownMyActivatedAbility(part.FangsActivatedAbilityID, part.GetCooldown(part.Level));
