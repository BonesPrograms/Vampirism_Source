...
namespace XRL.World.Parts.Mutation
{
	...
	public class Vampirism : BaseDefaultEquipmentMutation
	{
		...
		public int GetCooldown(int Level) => FEED.COOLDOWN;
		...
		public override bool HandleEvent(EndTurnEvent E)
		{
			...
			{
				if (WikiRng.Next(1, 10) == 10 && !Scan.SafeReturnProperty(ParentObject, Flags.FEED))
				{
		...
		void Start()
		{
			...
			if (TryGetTarget(Cell, out GameObject Target))
				new HandleCommand(Target, this, Biting.TryCreateInstance(ParentObject, Target, out Biting Bite), Bite).Initialize();
