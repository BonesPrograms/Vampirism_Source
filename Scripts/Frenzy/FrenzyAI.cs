...
namespace XRL.World.Effects
{
    ...
    public class FrenzyAI : Effect
    {
        ...
        =>
            Object == base.Object
            && !InRange
            && !Scan.ReturnProperty(base.Object, Flags.FEED) //fun bug here. because frenzy never uses energy, if you are attacked by a group, you will stack feeding on all of them and become god. so we check for if FEED == false before swapping targets
        ...
        void EndingTimer()
        {
            if (!gameover)
            {
                if (!activated && Scan.ReturnProperty(base.Object, Flags.FEED))
                    StartCountdown();
                if (activated)
                    Countdown();
            }
        ...
        {
            feedtime--;
            if (feedtime == 0 || !Scan.ReturnProperty(base.Object, Flags.FEED))
                Duration = 0;
        }
        ...
        void CheckBloodAndCooldown()
        {
            ...
            part.CooldownMyActivatedAbility(part.FangsActivatedAbilityID, Vampirism.GetCooldown(part.Level));
