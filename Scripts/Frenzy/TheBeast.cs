...
namespace XRL.World.Parts
{
	...
	public class TheBeast : IPart
	{
		...
		{
			if (Options.GetOptionBool(Nexus.Rules.OPTIONS.FRENZY) && ParentObject.IsPlayer() && ParentObject.GetPart<Vampirism>().HasFangs()) //actually prevents you from frenzying if dominating
			{
				Clean();
				if (!Scan.ReturnProperty(ParentObject, Flags.FEED) && !frenzied && !Scan.Incap(ParentObject, true))
		...
		void Clean()
		{
			List<GameObject> keys = new(TargetRegistry.Keys);
