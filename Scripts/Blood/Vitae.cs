...
namespace XRL.World.Parts
{
    ...
    {                           //even if dominating a vampire, their blood value will never change. might change this one day
        ...
        {
            if (!base.WantEvent(ID, cascade) && ID != PooledEvent<InduceVomitingEvent>.ID && ID != SingletonEvent<BeforeTakeActionEvent>.ID)
        ...
        {
            if (ParentObject.IsPlayer() && !AutoAct.IsResting() && !Scan.Incap(ParentObject, false) && !ParentObject.IsInCombat() && Options.GetOptionBool(OPTIONS.AUTOGET) && !Options.GetOptionBool(OPTIONS.HUNTER) && !Scan.ReturnProperty(ParentObject, Flags.FRENZY, Flags.FEED))
        ...
        public override bool HandleEvent(BeginTakeActionEvent E)
        {
            if (ParentObject.IsOriginalPlayerBody() && ParentObject.IsPlayer())
            {
                ...
                if (!Options.GetOptionBool(OPTIONS.HUNTER) && !Scan.ReturnProperty(ParentObject, Flags.FRENZY, Flags.FEED) && !Scan.Incap(ParentObject, false))
                    BloodAutoSip();
            }
        ...
        public string BloodStatus() => Scan.ReturnProperty(ParentObject, Flags.GO) ? "{{r|Bottomless}}" : Switch();
        string Switch() =>
        Blood switch
        {
        ...
        { //cannot know at compile time if you might be frenzying at any given moment
            if (!Scan.ReturnProperty(ParentObject, Flags.FRENZY) && !Scan.Incap(ParentObject, false) && ParentObject.IsPlayer())
            {
