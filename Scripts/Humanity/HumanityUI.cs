...
namespace XRL.World.Effects
{
    ...
    public class HumanityUI : Effect
    {
        ...
        string Regen(int humanity) => humanity != Nexus.Rules.Humanity.MAX ? "\nRegeneration: {{B sequence|" + base.Object.GetIntProperty(Flags.REGEN) + "}}/5000" : "\nRegeneration: {{G|Max}}";
        public override string GetDetails()
        {
            int humanity = base.Object.GetIntProperty(Flags.HUMANITY);
            return humanity switch
            {
                Nexus.Rules.Humanity.MAX => "{{G sequence|5}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                Nexus.Rules.Humanity.HIGH => "{{G sequence|4}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                Nexus.Rules.Humanity.MID => "{{W sequence|3}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                Nexus.Rules.Humanity.LOW => "{{W sequence|2}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                Nexus.Rules.Humanity.CRIT => "{{R sequence|1}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                Nexus.Rules.Humanity.GAMEOVER => "{{R sequence|Wight}}\nYou have given in to your inner animal, and have become wild.\nYou will never feel full again.",
