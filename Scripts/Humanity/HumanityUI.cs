...
namespace XRL.World.Effects
{
    ...
    internal class HumanityUI : Effect
    {
        ...
        string Regen(int humanity) => humanity != VampirismSys.Rules.Hum.MAX ? "\nRegeneration: {{B sequence|" + base.Object.GetIntProperty(Flags.REGEN) + "}}/5000" : "\nRegeneration: {{G|Max}}";
        public override string GetDetails()
        {
            int humanity = base.Object.GetIntProperty(Flags.HUMANITY);
            return humanity switch
            {
                VampirismSys.Rules.Hum.MAX => "{{G sequence|5}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                VampirismSys.Rules.Hum.HIGH => "{{G sequence|4}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                VampirismSys.Rules.Hum.MID => "{{W sequence|3}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                VampirismSys.Rules.Hum.LOW => "{{W sequence|2}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                VampirismSys.Rules.Hum.CRIT => "{{R sequence|1}}{{Y sequence|/5}} {{G sequence|Humanity}}" + Regen(humanity),
                VampirismSys.Rules.Hum.GAMEOVER => "{{R sequence|Wight}}\nYou have given in to your inner animal, and have become wild.\nYou will never feel full again.",
