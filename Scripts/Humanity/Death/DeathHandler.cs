...
namespace XRL.World.Parts
{
    ...
    public class DeathHandler : IPart
    {
        ...
        {
            if (Security() && !Scan.ReturnProperty(Player, Flags.GO) && Options.GetOptionBool(Nexus.Rules.OPTIONS.HUMANITY) && E?.Dying?.HasStringProperty(Flags.DEAD) is false)
                new Deaths(Player, E.Dying, E.Killer).Possibilities();
            return base.HandleEvent(E);
        }
